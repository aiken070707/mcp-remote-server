...
For more details about client primitives see client concepts. Besides server and client primitives, the protocol offers cross-cutting utility primitives that augment how requests are executed:
...
* "tools": {"listChanged": true} - The server supports the tools primitive AND can send tools/list\_changed notifications when its tool list changes.
...
When the server’s available tools change—such as when new functionality becomes available or existing tools are modified—the server can proactively notify connected clients.
